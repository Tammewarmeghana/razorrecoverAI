...
export default function RecoveryFunnel({ cases = [], metrics }) {
  ...
  const riskEvaluated = cases.filter(c => c.risk?.risk_score !== undefined || c.risk_score > 0).length || Math.round(totalFailed * 0.88);
  ...
  const funnelSteps = [
    { label: 'FAILED PAYMENTS', count: totalFailed, color: '#e11d48', pct: 100 },
    { label: 'RISK DETECTED', count: riskEvaluated, color: '#d97706', pct: totalFailed ? Math.round((riskEvaluated / totalFailed) * 100) : 0 },
    { label: 'AI DIAGNOSED', count: aiDiagnosed, color: '#4f46e5', pct: totalFailed ? Math.round((aiDiagnosed / totalFailed) * 100) : 0 },
    { label: 'ACTION EXECUTED', count: actionExecuted, color: '#0284c7', pct: totalFailed ? Math.round((actionExecuted / totalFailed) * 100) : 0 },
